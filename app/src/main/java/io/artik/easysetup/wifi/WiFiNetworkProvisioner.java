...
/*                Log.d(TAG, "Requesting scan to find valid Proximity network");
        ...
        protected void handleSupplicantStateChangedAction(final Intent intent) {
            ...
            if (mWifiManager != null) {
                if (detailedStateOf.equals(NetworkInfo.DetailedState.DISCONNECTED) ||
                        detailedStateOf.equals(NetworkInfo.DetailedState.FAILED)) {
                    if (mState == State.CONNECTING &&
                ...
                } else if (detailedStateOf.equals(NetworkInfo.DetailedState.AUTHENTICATING)) {
                    mProvisionerListener.onAuthenticating();
                } else if (detailedStateOf.equals(NetworkInfo.DetailedState.OBTAINING_IPADDR)) {
    ...
    private boolean setWifiApEnabled(WifiConfiguration config, boolean enable) {
        try {
            Method methods = mWifiManager.getClass().getMethod(Constants.SETWIFIAPENABLED, WifiConfiguration.class, boolean.class);
    ...
    private boolean isWifiApEnabled() {
        try {
            Method methods = mWifiManager.getClass().getMethod(Constants.ISWIFIAPENABLED);
